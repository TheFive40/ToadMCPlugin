...
public class ClassesRegistration {
    public void loadListeners(String packageName) {
        for (Class<?> clazz : getClassesInPackage(packageName)) {
            if (isListener(clazz)) {
                try {
                    ToadMC.instance.getServer().getPluginManager().registerEvents((Listener) clazz.newInstance(), ToadMC.instance);
    ...
    public Collection<Class<?>> getClassesInPackage(String packageName) {
        ...
        CodeSource codeSource = ToadMC.instance.getClass().getProtectionDomain().getCodeSource();
